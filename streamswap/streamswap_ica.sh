...
# make sure the token to stream is in the osmosis wallet
...
# retreive and fund the ICA account for fees (or use a hosted account when supported)
TRANSFER_TX_RES=$(./build/osmosisd tx bank send $OSMO_ADDRESS $ICA_ADDRESS 1000$INTO_OSMOSIS_DENOM --from $INTO_USER --node https://rpc.osmotest5.osmosis.zone --fees 1000uosmo --gas 400000 --chain-id osmo-test-5 -y)
...
cat <<EOF >"$msg_dca_file"
{
  "subscribe": {
    "stream_id": "$STREAM_ID",
    "operator": "$OSMO_ADDRESS",
    "operator_target": "ICA_ADDR"
  }
}
EOF
